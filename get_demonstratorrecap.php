...
//ADD FIND CRITERION IN REQUEST
$findRequest1->addFindCriterion('UIDRep', $uidDemonstrator);
//ADD FIND CRITERION IN REQUEST
...
if (FileMaker::isError($result)) {
    ...
    echo('<div id="replace_data"><p>No recaps found</p><p>Message: '.$message.'</p><button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-target="#datesearch">Dates</button></div>');
    die();
}
...
$title = $found.' Recaps for '.$found_records_row->getField($tbase.'ShowSchedule::rep');

if( $result=="No records match the request" ) { 
echo '<div id="replace_data"><p>No recaps found</p><p>[no recaps]</p><button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-target="#datesearch">Dates</button></div>';
exit();
}
?>  
